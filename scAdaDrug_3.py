...
class Model(nn.Module):
    ...
    def forward(self, s1x, s2x, s3x, tx):
        s1_decoded, s1_encoded = self.autoencoder(s1x)
        s2_decoded, s2_encoded = self.autoencoder(s2x)
        s3_decoded, s3_encoded = self.autoencoder(s3x)
        t_decoded, t_encoded = self.autoencoder(tx)

        s1_pred = self.predictor(s1_encoded)
        s2_pred = self.predictor(s2_encoded)
        s3_pred = self.predictor(s3_encoded)
        t_pred = self.predictor(t_encoded)

        L1_s1_t = t_encoded - s1_encoded
        L1_s2_t = t_encoded - s2_encoded
        L1_s3_t = t_encoded - s3_encoded
        
        fv1 = self.fv_generator(L1_s1_t)
        fv2 = self.fv_generator(L1_s2_t)
        fv3 = self.fv_generator(L1_s3_t)
        
        cos_sim12 = F.cosine_similarity(fv1, fv2, dim=1)
        corr12 = cos_sim12.pow(2)
        cos_sim23 = F.cosine_similarity(fv2, fv3, dim=1)
        corr23 = cos_sim23.pow(2)
        cos_sim13 = F.cosine_similarity(fv1, fv3, dim=1)
        corr13 = cos_sim13.pow(2)
        corr = corr12 + corr13 + corr23
        ...
        st_encoded = torch.cat((fs1, fs2, fs3, ft), dim=0)

        domain_pre = self.domain_classifier(st_encoded)

        return corr, s1_encoded, s1_decoded, s1_pred, s2_encoded, s2_decoded, s2_pred, s3_encoded, s3_decoded, s3_pred, t_encoded, t_decoded, t_pred, domain_pre
