...
for epoch in range(n_epochs):
    ...
    for i, data in enumerate(zip(source_train_loader, cycle(target_train_loader))):
        if (data[0][0].size()[0] != bs * 2) | (data[1][0].size()[0] != bs):
            continue

        sx = data[0][0].to(device)
        s1x = sx[::2]
        s2x = sx[1::2]
        ssx = torch.cat((s1x, s2x), dim=1)

        sy = data[0][1].to(device)
        s1y = sy[::2]
        s2y = sy[1::2]
        sy = torch.cat((s1y, s2y), dim=0)

        tx = data[1][0].to(device)
        ty = data[1][1].to(device)

        corr, s1_encoded, s1_decoded, s1_pred, s2_encoded, s2_decoded, s2_pred, t_encoded, t_decoded, t_pred, domain_pre = model(
            s1x, s2x, tx)

        Labels = torch.ones(bs * 2, 1).to(device)
        ...
        reconstruction_losses = reconstruction_loss(s1_decoded, s1x) + reconstruction_loss(s2_decoded, s2x)
        s1_pred = s1_pred.squeeze()
        s2_pred = s2_pred.squeeze()
        s_pred = torch.cat((s1_pred, s2_pred), dim=0)
        prediction_losses = prediction_loss(s1_pred, s1y) + prediction_loss(s2_pred, s2y)
